...
res_filename = 'resources.csv'


train_data_raw = pd.read_csv(train_filename, parse_dates=["project_submitted_datetime"], low_memory=False, dtypes=).fillna('')
test_data_raw = pd.read_csv(test_filename, parse_dates=["project_submitted_datetime"], low_memory=False).fillna('')
resource_data_raw = read_csv(res_filename, low_memory=False).fillna('')
